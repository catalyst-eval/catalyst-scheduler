...
/**
 * Run the combined daily task manually
 * 
 * GET /api/scheduling/run-daily-task
 */
router.get('/run-daily-task', async (req: Request, res: Response) => {
  try {
    // Create scheduler service
    const schedulerService = new SchedulerService();
    
    // Run the combined daily task
    console.log('API request to run the combined daily task');
    const result = await schedulerService.combinedDailyTask();
    ...
    console.error('Error running combined daily task:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    });
